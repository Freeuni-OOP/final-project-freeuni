...
import com.oophotel.dao.PoolReservationDao;
...
@WebServlet("/api/pool/reservations")
public class PoolServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (body == null || isBlank(body.name) || isBlank(body.email)
                || isBlank(body.pool) || isBlank(body.date) || isBlank(body.time)) {
            JsonUtil.write(response, 400, "Name, email, pool, date and time are required");
            return;
        }
        ...
        int capacity = PoolReservationDao.capacityFor(body.pool);
        if (capacity == 0) {
            JsonUtil.write(response, 400, "Unknown pool selected");
            return;
        }

        try {
            PoolReservation reservation = dao.create(body.name.trim(), body.email.trim(),
                    body.pool, body.activity, date, time, guests, body.notes);
